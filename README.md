# 日程ピッカー (Meeting Slot Picker)

`Ctrl+Shift+U` でどこからでも開けるメニューバー常駐アプリ。Google カレンダーの予定を重ねた週グリッドで空き枠（30分単位）を選ぶと、日程調整用のテキストがクリップボードに入る。
...
- **Google カレンダー連携**（読み取り専用 `calendar.readonly`）— 既存予定を重ね、空き時間から選ぶ
...
- macOS 11 (Big Sur) 以上 / Intel・Apple Silicon どちらも対応 (Universal Binary)
