...
- **2026-07-06 UI 全面リフレッシュ (`feat(ui)`)**: SaaS 品質の見た目に刷新＋**ダークモード対応（OS 追従、3配布形態共通）**。デザイントークン（brand スケール/影/アニメ/btn・input 等の共通クラス @ styles.css）、ヘッダ（ロゴ+プロバイダ接続グリフ+kbd チップ）、グリッド（今日ハイライト・現在時刻ライン NowIndicator・左アクセントのイベントバー）、オンボーディング（プロバイダカード+グリフ）、設定（セクション化・接続済みピル・トグルスイッチ・sticky フッタ）。**機能・文言・操作は不変**（変えたのは event-bar 色 pin のみ）。264 テスト・e2e・2ビルド緑、ライト/ダーク×デスクトップ/モバイル/設定/オンボーディングをスクショ目視検証済み
